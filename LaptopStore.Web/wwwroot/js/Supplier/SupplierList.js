...
function DeleteSupplier() {
    if (deleteId) {
        baseDelete('/Supplier/DeleteSupplier/' + deleteId).then(res => {
...
function getDataByPaging() {
    ...
    baseGetDataFilterPaging('/Supplier/GetSupplierPaging', paging).then(res => {
        if (res.code === 200) {
...
function renderDataList(data) {
    ...
    if (data?.length > 0) {
        $.each(data, function (index, item) {
            ...
            let updateUrl = 'Supplier/Update?id=' + item.id;
            let detailUrl = 'Supplier/Detail?id=' + item.id;
...
function handleViewDetail(id) {
    baseGetPartialView('Supplier/GetDetail/' + id).then(res => {
        $('#supplierDetailBody').html(res);
