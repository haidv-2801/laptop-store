...
function DeleteCustomer() {
    if (deleteId) {
        baseDelete('/Customer/DeleteCustomer/' + deleteId).then(res => {
...
function getDataByPaging() {
    ...
    baseGetDataFilterPaging('/Customer/GetCustomerPaging', paging).then(res => {
        if (res.code === 200) {
...
function renderDataList(data) {
    ...
    if (data?.length > 0) {
        $.each(data, function (index, item) {
            ...
            let updateUrl = 'Customer/Update?id=' + item.id;
            let detailUrl = 'Customer/Detail?id=' + item.id;
...
function handleViewDetail(id) {
    baseGetPartialView('Customer/GetDetail/' + id).then(res => {
